...
async function geocodeLocation(location) {
  const places = {
    'houston':[29.7604,-95.3698], 'pasadena':[29.6911,-95.2091],
    'deer park':[29.7052,-95.1238], 'la porte':[29.6658,-95.0194],
    'seabrook':[29.5633,-95.0255], 'kemah':[29.5427,-95.0205],
    'league city':[29.5075,-95.0949], 'webster':[29.5377,-95.1183],
    'friendswood':[29.5294,-95.2010], 'pearland':[29.5636,-95.2860],
    'alvin':[29.4238,-95.2441], 'santa fe':[29.3780,-95.1058],
    'dickinson':[29.4608,-95.0513], 'texas city':[29.3838,-94.9027],
    'la marque':[29.3686,-94.9713], 'galveston':[29.3013,-94.7977],
    'baytown':[29.7355,-94.9774], 'humble':[29.9988,-95.2622],
    'kingwood':[30.0575,-95.1903], 'spring':[30.0799,-95.4172],
    'the woodlands':[30.1658,-95.4613], 'conroe':[30.3119,-95.4561],
    'willis':[30.4249,-95.4799], 'montgomery':[30.3880,-95.6963],
    'magnolia':[30.2094,-95.7508], 'tomball':[30.0972,-95.6161],
    'cypress':[29.9691,-95.6972], 'katy':[29.7858,-95.8245],
    'richmond':[29.5822,-95.7608], 'rosenberg':[29.5572,-95.8086],
    'sugar land':[29.6197,-95.6349], 'missouri city':[29.6186,-95.5377],
    'stafford':[29.6161,-95.5577], 'fulshear':[29.6899,-95.8997],
    'angleton':[29.1694,-95.4319], 'lake jackson':[29.0339,-95.4344],
    'clute':[29.0247,-95.3988], 'freeport':[28.9541,-95.3597],
    'surfside beach':[28.9441,-95.2888], 'west columbia':[29.1439,-95.6452]
  };
  const l = (location || '').toLowerCase().replace(/\s+/g,' ').trim();
  if (!/\b(tx|texas)\b/.test(l)) return null;
  const names = Object.keys(places).sort((a,b)=>b.length-a.length);
  for (const k of names) {
    if (l.includes(k)) return {lat:places[k][0], lon:places[k][1], city:k};
  }
  return null;
}
...
async function verifyAndParse(url) {
  ...
  try {
    ...
    if (x.distance_miles > RADIUS) return null;
...
function saveListing(x){
  ...
  x = {
    source: '', source_id: null, url: '', title: '', year: null, make: '', model: '',
    location: '', latitude: null, longitude: null, distance_miles: null,
    asking_price: null, length_ft: null, horsepower: null, engine_make: '',
    engine_hours: null, seating: null, has_ttop: 0, ttop_type: '',
    has_trolling: 0, has_powerpole: 0, has_jackplate: 0, trailer: '',
    estimated_new_low: null, estimated_new_high: null, fair_value_low: null,
    fair_value_high: null, accessories_new_low: null, accessories_new_high: null,
    accessories_today_low: null, accessories_today_high: null, cost_to_complete: 0,
    all_in: null, deal_score: 0, why: '', raw_excerpt: '', ...x
  };
...
async function runSearch(){
  const queries = [
    'site:boattrader.com/boat/ (Houston OR Kemah OR Seabrook OR Galveston OR Conroe OR Freeport) TX bay boat T-top trolling motor Power-Pole jack plate 150 200',
    'site:boats.com (Houston OR Kemah OR Seabrook OR Galveston OR Conroe OR Freeport) TX bay boat T-top trolling motor Power-Pole jack plate',
    'site:yachtworld.com/yacht/ (Houston OR Kemah OR Seabrook OR Galveston OR Conroe OR Freeport) TX bay boat T-top trolling motor Power-Pole'
  ];
  let urls = [];
